...
[ApiController]
[Route("api/matches")]
public class MatchesController(AppDbContext db, IHubContext<ScoreHub> hub, IMatchRunTime rt, ITeamDirectory teams) : ControllerBase
{
        ...
        [FromQuery] DateTime? to = null)
    {
        ...
        if (matches.Count == 0)
            return Ok(new { items = Array.Empty<object>(), total, page, pageSize });

        var matchIds = matches.Select(m => m.Id).ToList();
        var fouls = await db.Fouls
            .Where(f => matchIds.Contains(f.MatchId))
            .GroupBy(f => new { f.MatchId, f.TeamId })
            .Select(g => new { g.Key.MatchId, g.Key.TeamId, Count = g.Count() })
            .ToListAsync();

        var foulLookup = fouls.ToDictionary(x => (x.MatchId, x.TeamId), x => x.Count);

        var teamIds = matches.SelectMany(m => new[] { m.HomeTeamId, m.AwayTeamId });
        var teams = await _teams.GetTeamsAsync(teamIds);
        ...
        var items = matches.Select(m => new
        {
            ...
            homeTeam = ResolveTeamName(teams, m.HomeTeamId),
            awayTeam = ResolveTeamName(teams, m.AwayTeamId),
            homeScore = m.HomeScore,
            awayScore = m.AwayScore,
            quarter = m.Period,
            quarterDurationSeconds = m.QuarterDurationSeconds,
            homeFouls = foulLookup.TryGetValue((m.Id, m.HomeTeamId), out var hf) ? hf : 0,
            awayFouls = foulLookup.TryGetValue((m.Id, m.AwayTeamId), out var af) ? af : 0
        }).ToList();
    ...
    {
        var m = await db.Matches.FirstOrDefaultAsync(x => x.Id == id);
        ...
        var teams = await _teams.GetTeamsAsync(new[] { m.HomeTeamId, m.AwayTeamId });
        var homeTeam = ResolveTeamName(teams, m.HomeTeamId);
        var awayTeam = ResolveTeamName(teams, m.AwayTeamId);
    ...
    public async Task<IActionResult> Programar([FromBody] ProgramarPartidoDto dto)
    {
        ...
        var home = await _teams.GetTeamAsync(dto.HomeTeamId);
        var away = await _teams.GetTeamAsync(dto.AwayTeamId);
    ...
    public async Task<IActionResult> Proximos()
    {
        ...
        var teams = await _teams.GetTeamsAsync(matches.SelectMany(x => new[] { x.HomeTeamId, x.AwayTeamId }));

        var list = matches.Select(x => new
        {
            id = x.Id,
            homeTeamId = x.HomeTeamId,
            awayTeamId = x.AwayTeamId,
            homeTeam = ResolveTeamName(teams, x.HomeTeamId),
            awayTeam = ResolveTeamName(teams, x.AwayTeamId),
    ...
    public async Task<IActionResult> NewGame([FromBody] NewGameDto dto)
    {
        ...
        var homeName = dto.HomeName.Trim();
        var awayName = dto.AwayName.Trim();

        var homeId = await _teams.CreateTeamAsync(homeName, null);
        var awayId = await _teams.CreateTeamAsync(awayName, null);

        if (homeId is null || awayId is null)
            return StatusCode(502, "No se pudieron registrar los equipos");

        var match = new MatchEntity
        {
            HomeTeamId = homeId.Value,
            AwayTeamId = awayId.Value,
            Status = "Scheduled",
            QuarterDurationSeconds = dto.QuarterDurationSeconds is > 0 ? dto.QuarterDurationSeconds!.Value : 10,
            HomeScore = 0,
            AwayScore = 0,
    ...
    private static string ResolveTeamName(IDictionary<int, TeamSummary> teams, int id)
        => teams.TryGetValue(id, out var team) ? team.Name : $"Equipo #{id}";
}
